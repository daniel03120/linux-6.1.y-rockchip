...
static int yt8512_clk_init(struct phy_device *phydev)
{
	...
	val = ytphy_read_ext_with_lock(phydev, YT8512_EXTREG_AFE_PLL);
	if (val < 0)
		return val;

	val |= YT8512_CONFIG_PLL_REFCLK_SEL_EN;

	ret = ytphy_write_ext_with_lock(phydev, YT8512_EXTREG_AFE_PLL, val);
	if (ret < 0)
		return ret;

	val = ytphy_read_ext_with_lock(phydev, YT8512_EXTREG_EXTEND_COMBO);
	if (val < 0)
		return val;

	val |= YT8512_CONTROL1_RMII_EN;

	ret = ytphy_write_ext_with_lock(phydev, YT8512_EXTREG_EXTEND_COMBO, val);
	if (ret < 0)
		return ret;
...
static int yt8512_led_init(struct phy_device *phydev)
{
	...
	val = ytphy_read_ext_with_lock(phydev, YT8512_EXTREG_LED0);
	if (val < 0)
		return val;
	...
	ret = ytphy_write_ext_with_lock(phydev, YT8512_EXTREG_LED0, val);
	if (ret < 0)
		return ret;

	val = ytphy_read_ext_with_lock(phydev, YT8512_EXTREG_LED1);
	if (val < 0)
		return val;
	...
	ret = ytphy_write_ext_with_lock(phydev, YT8512_LED1_BT_ON_EN, val);
